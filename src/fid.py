...
def calculate_activation_statistics(isreal, dataloader, num_samples, model, device, dims=2048
    ...
    Input:
        model(nn.moduel): Instance of inception model
        dims(int):Dimensionality of features returned by Inception
        device(torch.device)Device to run calculations
    Outputs:
    Outputs
        mu (np.array): mean of pool_3 layer of Inceptionv3 activation on the dataloader data
        sigma (np.array): std of pool_3 layer of Inceptionv3 activation on the dataloader data
    """
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    Inputs :
        mu1(np.array) : mean of pool_3 layer of Inceptionv3 activation on the real data
        sigma1(np.array) :  std of pool_3 layer of Inceptionv3 activation on the real data
        mu2(np.array) : mean of pool_3 layer of Inceptionv3 activation on the fake data
        sigma2(np.array) :  std of pool_3 layer of Inceptionv3 activation on the fake data
        eps(float) : for numericla stability
    Outputs :
        FID(np.array): FID evaluation as scalar 
    """
...
def calculate_frechet(device, real_dataloader, fake_dataloader, inception_model, num_samples):
    '''
    wrapper to calculate the activation statistics of two datasets (given by dataloaders)
    and then calculate the FID between the two datasets

    Inputs:
        device(torch.Device) : device to use for the computations
        real_dataloader(DataLoaderclass) : dataloader containing the real samples
        fake_dataloader(DataLoaderclass) : dataloader containing the fake samples
        inception_model(nn.Module) : the inceptionv3 model
        num_samples(int): number of samples in each dataset
    Outputs:
        fid_value(np.array):FID distance
    '''
...
def sample_gen_dataset(n_samples, batch_size, netG, nz, workers, device, shuffle=True):
    '''
    NOTE: PLEASE SET THE NUMBER OF WORKERS TO 0 EVERYWHERE IF EVALUATING FID WITH GPU
    genreate a dataloader containing n fake samples generated by the generator netG

    Inputs:
        n_samples, batch_size, netG, nz, workers, device, shuffle=True
    Outputs
        fake_dataloader(DataLoaderclass) : dataloader containing the fake samples
...
def calculate_fid(num_samples, real_dataloader, batch_size_eval, device, inception_model, netG, nz, workers):
    '''
    wrapper function used to generate a fake sample and then calculate the FID
    '''
...
def get_fid_scores(ngpu, num_samples, real_dataloader, batch_size_eval, device, inception_model, nc, nz, workers,
    ...
    '''
    Function used to load the models from their save files in generated_data, then evaluate them
    using the FID score
    '''
    all_lr_scores = {}
